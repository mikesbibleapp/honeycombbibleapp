...
(function () {
  ...
  const WHO_SAID_IT = [
    { quote: "I am the way, the truth, and the life.", speaker: "Jesus", books: ["John"] },
    { quote: "Let my people go.", speaker: "Moses", books: ["Exodus"] },
    { quote: "The LORD is my shepherd; I shall not want.", speaker: "David", books: ["Psalms"] },
    { quote: "Here am I; send me.", speaker: "Isaiah", books: ["Isaiah"] },
    { quote: "Am I my brother's keeper?", speaker: "Cain", books: ["Genesis"] },
    { quote: "Whither thou goest, I will go.", speaker: "Ruth", books: ["Ruth"] },
    { quote: "Prepare ye the way of the Lord.", speaker: "John the Baptist", books: ["Matthew", "Mark", "Luke", "John"] },
    { quote: "I have fought the good fight, I have finished the race.", speaker: "Paul", books: ["2 Timothy"] },
    { quote: "Why have you forsaken me?", speaker: "Jesus", books: ["Matthew", "Mark"] },
    { quote: "I am the resurrection and the life.", speaker: "Jesus", books: ["John"] },
    { quote: "Be still, and know that I am God.", speaker: "God", books: ["Psalms"] },
    { quote: "I am that I am.", speaker: "God", books: ["Exodus"] },
    { quote: "Behold, the Lamb of God.", speaker: "John the Baptist", books: ["John"] },
    { quote: "Father, forgive them; for they know not what they do.", speaker: "Jesus", books: ["Luke"] },
    { quote: "It is finished.", speaker: "Jesus", books: ["John"] },
    { quote: "I will arise and go to my father.", speaker: "Prodigal Son", books: ["Luke"] },
    { quote: "Faith without works is dead.", speaker: "James", books: ["James"] },
    { quote: "Stand still, and see the salvation of the LORD.", speaker: "Moses", books: ["Exodus"] },
    { quote: "Choose you this day whom ye will serve.", speaker: "Joshua", books: ["Joshua"] },
    { quote: "Speak, LORD, for thy servant heareth.", speaker: "Samuel", books: ["1 Samuel"] },
    { quote: "I am a man of unclean lips.", speaker: "Isaiah", books: ["Isaiah"] },
    { quote: "By their fruit you will recognize them.", speaker: "Jesus", books: ["Matthew"] },
    { quote: "Love your enemies.", speaker: "Jesus", books: ["Matthew", "Luke"] },
    { quote: "Truly this man was the Son of God.", speaker: "Roman Centurion", books: ["Mark"] },
    { quote: "I can do all things through Christ who strengthens me.", speaker: "Paul", books: ["Philippians"] },
    { quote: "Blessed are the poor in spirit.", speaker: "Jesus", books: ["Matthew"] },
    { quote: "Have I not commanded you? Be strong and courageous.", speaker: "God", books: ["Joshua"] },
    { quote: "If God is for us, who can be against us?", speaker: "Paul", books: ["Romans"] },
    { quote: "We do not live by bread alone.", speaker: "Jesus", books: ["Matthew", "Luke"] },
    { quote: "Vanity of vanities; all is vanity.", speaker: "Solomon", books: ["Ecclesiastes"] },
    { quote: "Behold, I make all things new.", speaker: "Jesus", books: ["Revelation"] },
    { quote: "Today salvation has come to this house.", speaker: "Jesus", books: ["Luke"] },
    { quote: "Get behind me, Satan!", speaker: "Jesus", books: ["Matthew", "Mark"] },
    { quote: "I am with you always, even to the end of the age.", speaker: "Jesus", books: ["Matthew"] },
    { quote: "How long, O LORD?", speaker: "David", books: ["Psalms"] },
    { quote: "What is truth?", speaker: "Pilate", books: ["John"] },
    { quote: "Where two or three are gathered, I am there.", speaker: "Jesus", books: ["Matthew"] },
    { quote: "My grace is sufficient for you.", speaker: "God", books: ["2 Corinthians"] },
    { quote: "Even so, come, Lord Jesus.", speaker: "John", books: ["Revelation"] },
    { quote: "Naked I came from my mother's womb, and naked shall I return.", speaker: "Job", books: ["Job"] },
  ];
  ...
  function versePool(bible, plan, seed) {
    ...
    const contextPlan = [];
    for (let i = Math.max(0, dayStart - 4); i < Math.min(plan.length, dayStart + 8); i++) {
    ...
    contextPlan.forEach(([bk, ch]) => {
      ...
      chapter.forEach((row) => {
        const text = row[1];
        if (typeof text === "string" && text.length >= 45 && text.length <= 220) {
  ...
  function finishVerseQuestions(bible, plan, seed, rounds) {
    ...
    const primary = pool.context.length >= rounds ? pool.context : pool.context.concat(pool.all);
    const questions = [];
    const used = new Set();
    primary.forEach((v) => {
      ...
      used.add(id);
      const wrong = fillWrongEndings(Object.assign({}, v, split), pool.context.concat(pool.all), pool.rng);
      if (wrong.length < 3) return;
  ...
  function matchBookQuestions(bible, plan, seed, rounds) {
    ...
    const preferred = pool.context.length >= rounds ? pool.context : pool.context.concat(pool.all);
    const questions = [];
    const used = new Set();
    preferred.forEach((v) => {
      if (questions.length >= rounds) return;
      ...
      if (used.has(id)) return;
      used.add(id);
      const distractorBase = shuffle(pool.contextBooks.concat(allBooks), pool.rng);
      const wrong = [];
      distractorBase.forEach((b) => {
        if (wrong.length < 3 && b !== v.book && !wrong.includes(b)) wrong.push(b);
      });
      if (wrong.length < 3) return;
  ...
  function whoSaidQuestions(bible, plan, seed, rounds) {
    ...
    source.forEach((q) => {
      ...
      shuffle(speakers, pool.rng).forEach((s) => {
        if (wrong.length < 3 && s !== q.speaker && !wrong.includes(s)) wrong.push(s);
      });
      if (wrong.length < 3) return;
